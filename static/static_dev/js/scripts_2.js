...
formValidation = {
	...
	submitFormHandler: function(e){
		...
		if(this.validatedFields.firstName && this.validatedFields.lastName && this.validatedFields.email && this.validatedFields.password){
			...
			this.$submitButton.text('Пожалуйста исправь ошибки');
			setTimeout((function(){
				if(this.$submitButton.text() == 'Пожалуйста исправь ошибки'){
	...
	validateText: function($input){
		...
		if($input.val() != ''){
			...
			$input.parent().find('span.label-text').append(' <small class="error">(Поле пустое)</small>');
			return false;
		}
	},
    ...
    if(regEx.test($input.val())){
			...
			$input.parent().addClass('invalid');
			$input.parent().find('span.label-text').append(' <small class="error">(Email не доступен)</small>');
			return false;
		}
	},
	validatePassword: function($input){
		...
		if($input.val().length >= 8){
			...
			$input.parent().addClass('invalid');
			$input.parent().find('span.label-text').append(' <small class="error">(Пароль должен содержать более 7 символов)</small>');
			return false;
		}
	}
}.init();